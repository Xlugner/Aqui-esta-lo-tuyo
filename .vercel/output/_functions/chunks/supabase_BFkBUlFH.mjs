...
async function getProductBySlug(slug) {
  ...
  const { data: imagesData, error: imagesError } = await supabase.from("product_images").select("*").eq("product_id", productData.id);
  if (imagesError) {
    console.error("Error fetching product images:", imagesError);
  }
  return {
