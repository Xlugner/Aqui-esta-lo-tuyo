...
export const QuantitySelector = ({ product }: QuantitySelectorProps) => {
  ...
  // Obtener número de WhatsApp desde storeConfig
  useEffect(() => {
    getStoreConfig().then(config => {
      const number = config?.whatsapp_number || import.meta.env.PUBLIC_WHATSAPP_NUMBER;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <a
          href={whatsappNumber ? `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(`Hola! Me interesa: ${product.name} (cantidad: ${quantity})`)}` : '#'}
          target={whatsappNumber ? "_blank" : "_self"}
          rel="noopener noreferrer"
          className={`flex-1 font-bold py-2 sm:py-3 px-4 sm:px-6 rounded-lg transition-colors flex items-center justify-center gap-1 sm:gap-2 text-sm sm:text-base ${whatsappNumber ? 'bg-green-500 hover:bg-green-600 text-white' : 'bg-gray-400 text-gray-200 cursor-not-allowed'}`}
        >
